...
# from common.constants.generic_messages import SAVA_GENERIC_MESSAGE
# TODO ajout un fichier constants pour les messages génériques et les constantes
SAVAI_GENERIC_MESSAGE = "Désolé, je ne suis pas en mesure de répondre à votre demande pour le moment. Veuillez reformuler votre question ou essayer plus tard."
...
class ErrorCode(str, Enum):
    ...
    SEVERAL_NAMES_ERROR = status.HTTP_400_BAD_REQUEST
...
class BaseAppException(Exception):
    def __init__(self, message: str, error_code: ErrorCode,level: str  ,status_code : status = None, capture_traceback: bool = False, exact_message: bool = True):
        self.message = message
        self.error_code = error_code
        self.status_code = status_code if status_code else  getattr(error_code, 'value', 500)
...
class TechnicalException(BaseAppException):
    ...
    def __init__(self, message: str ,error_code: ErrorCode):
        ...
        super().__init__(message,error_code=error_code,level=level, capture_traceback=True)
...
class FunctionalException(BaseAppException):
    ...
    def __init__(self, message: str , error_code: ErrorCode, exact_message: bool = True):
        ...
        self.exact_message = exact_message
        super().__init__(message, error_code=error_code, level=self.level, exact_message=exact_message)
...
class GenericException(BaseAppException):
    ...
    def __init__(self, message: str, status_code: int, error_code: ErrorCode):
        self.level = __class__.__name__
        super().__init__(message, error_code=error_code,level=self.level, status_code=status_code,capture_traceback=False)
...
class ChatGPTResponseException(TechnicalException):
    ...
    def __init__(self, detail: str = None):
        message = f"Une erreur est survenue lors de la communication avec ChatGPT : {detail}"
...
class DataNotFound(FunctionalException):
    ...
    def __init__(self, param_fields: str | dict[str, str] | list[dict[str, str]]):
        if isinstance(param_fields, str):
            formatted_message = f"La donnée spécifiée est introuvable pour : {param_fields}"
            ...
            formatted_message = "La donnée spécifiée est introuvable pour : " + " | ".join(
                self._format_dict(entry) if isinstance(entry, dict) else entry
                for entry in param_fields
            )
        else:
            raise ValueError("param_fields doit être une chaîne, un dictionnaire ou une liste de dictionnaires")
...
class MissingData(FunctionalException):
    ...
    def __init__(self, param_fields: list[str]):
        formatted_message = f"Certaines data sont manquantes : {', '.join(param_fields)}"
        super().__init__(formatted_message, error_code=ErrorCode.MISSING_DATA)
        self.param_fields = param_fields
...
class NoMatchingAction(FunctionalException):
    ...
    def __init__(self, detail : str = None, exact_message: bool = False):
        message = SAVAI_GENERIC_MESSAGE
        super().__init__(message, ErrorCode.NO_MATCHING_ACTION, exact_message=exact_message)
        self.detail = detail
...
class FormatException(FunctionalException):
    ...
    def __init__(self, error_type: str = None):
        ...
        if error_type == "json":
            ...
            message = "Erreur de format ou de structure inconnue dans la réponse OpenAI."
        
        super().__init__(message, error_code=ErrorCode.FORMAT_ERROR)
        self.error_type = error_type

class SeveralNamesException(FunctionalException):
    """
    Exception thrown when there is several names in the response
    """
    def __init__(self):
        super().__init__("Plusieurs noms et prénoms ont été détéctés", error_code=ErrorCode.SEVERAL_NAMES_ERROR, is_error=False)
...
class UnknownEmployee(FunctionalException):
    ...
    def __init__(self, message: str):
        super().__init__(message, error_code=ErrorCode.MISSING_DATA)
...
class UnexpectedException(GenericException):
    ...
    def __init__(self, detail : str = None  ):
        ...
        super().__init__(message,status_code= ErrorCode.UNEXPECTED_ERROR.value, error_code= ErrorCode.UNEXPECTED_ERROR)
...
class NoEntity(GenericException):
    ...
    """
    def __init__(self):
        message = f"Impossible d'afficher la fiche contact RH. La personne recherchée n'est rattachée à aucune entité. Veuillez vérifier les informations saisies ou contacter le support RH pour plus d’assistance."
        super().__init__(message, status_code=ErrorCode.NO_ENTITY.value, error_code=ErrorCode.NO_ENTITY)